...
// Meme Template Table - stores meme templates for text-to-meme generation
export const memeTemplate = table(
  'meme_template',
  {
    id: text('id').primaryKey(),
    userId: text('user_id').references(() => user.id, { onDelete: 'set null' }), // null for system templates
    slug: text('slug').unique().notNull(),
    name: text('name').notNull(),
    description: text('description'),
    imageUrl: text('image_url').notNull(),
    thumbnailUrl: text('thumbnail_url'),
    // Text areas configuration (JSON array of text box positions)
    textAreas: text('text_areas'), // JSON: [{x, y, width, height, defaultText, fontSize, fontColor, ...}]
    // Semantic search fields for AI matching
    tags: text('tags'), // JSON array: ["work", "monday", "tired"]
    emotions: text('emotions'), // JSON array: ["frustrated", "sarcastic", "happy"]
    useCases: text('use_cases'), // JSON array: ["comparison", "reaction", "announcement"]
    category: text('category'), // e.g., "reaction", "comparison", "classic"
    source: text('source').notNull().default('system'), // system, user, ai
    status: text('status').notNull().default('active'), // active, pending, deleted
  ...
  (table) => [
    // Query active templates by category
    index('idx_meme_template_status_category').on(table.status, table.category),
    // Query templates by source (system, user, ai)
    index('idx_meme_template_source_status').on(table.source, table.status),
    // Order by popularity
    index('idx_meme_template_usage_count').on(table.usageCount),
  ]
);

// Meme Table - stores user-generated memes
export const meme = table(
  'meme',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
    ...
    imageUrl: text('image_url').notNull(),
    thumbnailUrl: text('thumbnail_url'),
    prompt: text('prompt'), // Original user input for AI generation
    textContent: text('text_content'), // JSON: final text content on the meme
    generationType: text('generation_type').notNull(), // ai, template, upload
    aiTaskId: text('ai_task_id'), // Reference to AI task if generated by AI
  ...
  (table) => [
    ...
    // Query public memes for community feed
    index('idx_meme_public_status').on(table.isPublic, table.status),
    // Order by popularity for trending
    index('idx_meme_like_count').on(table.likeCount),
    // Query by generation type
    index('idx_meme_generation_type').on(table.generationType, table.status),
  ]
);

// Meme Like Table
export const memeLike = table(
  'meme_like',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
  ...
  (table) => [
    // Unique constraint: user can only like a meme once
    index('idx_meme_like_user_meme').on(table.userId, table.memeId),
    // Query all likes for a meme
    index('idx_meme_like_meme_id').on(table.memeId),
  ]
);

// Meme Favorite Table
export const memeFavorite = table(
  'meme_favorite',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
  ...
  (table) => [
    // Unique constraint: user can only favorite a meme once
    index('idx_meme_favorite_user_meme').on(table.userId, table.memeId),
    // Query user's favorites
    index('idx_meme_favorite_user_id').on(table.userId),
  ]
);

// Meme Comment Table
export const memeComment = table(
  'meme_comment',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
  ...
  (table) => [
    // Query comments for a meme
    index('idx_meme_comment_meme_status').on(table.memeId, table.status),
    // Query user's comments
    index('idx_meme_comment_user_id').on(table.userId),
  ]
);
